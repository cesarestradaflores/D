// -----------------------------------------------------------------
// --- Player.js (VERSIÓN MEJORADA - Colisiones Sphere-based)
// -----------------------------------------------------------------
...
export class Player {
    constructor(scene, assets) {
        ...
        // NUEVO: Sistema de colisiones mejorado (esferas múltiples)
        this.hitSpheres = [];
        this.boundingBox = new THREE.Box3(); // Mantener por compatibilidad
        ...
        if (assets.playerModel) {
            ...
            // NUEVO: Inicializar hit spheres después de cargar modelo
            this.setupHitSpheres();
    ...
    // NUEVO: Sistema de hit spheres para colisiones más precisas
    setupHitSpheres() {
        // Crear esferas para diferentes partes del cuerpo
        // Podrías ajustar estas posiciones basándote en tu modelo
        this.hitSpheres = [
            {
                name: 'body',
                radius: Config.COLLISION_SETTINGS.PLAYER_RADIUS,
                offset: new THREE.Vector3(0, 1.0, 0) // Centro del cuerpo
            },
            {
                name: 'head',
                radius: Config.COLLISION_SETTINGS.PLAYER_RADIUS * 0.7,
                offset: new THREE.Vector3(0, 1.8, 0) // Cabeza
            }
        ];
        
        // DEBUG: Visualizar hit spheres (descomentar para debug)
        // this.createDebugSpheres();
    }
    
    createDebugSpheres() {
        this.hitSpheres.forEach((sphere, index) => {
            ...
            sphere.debugMesh = debugSphere;
        });
    }
    
    getWorldHitSpherePositions() {
        const positions = [];
        const worldPosition = new THREE.Vector3();
        
        this.hitSpheres.forEach(sphere => {
            // Calcular posición mundial de la esfera
            const position = sphere.offset.clone();
            position.applyMatrix4(this.group.matrixWorld);
            positions.push({
                position: position,
                radius: sphere.radius
            });
            
            // Actualizar mesh de debug si existe
            if (sphere.debugMesh) {
                sphere.debugMesh.position.copy(sphere.offset);
            }
        });
    ...
    checkSphereCollision(otherPosition, otherRadius) {
        const hitSpheres = this.getWorldHitSpherePositions();
        
        for (const hitSphere of hitSpheres) {
            const distance = hitSphere.position.distanceTo(otherPosition);
            if (distance < (hitSphere.radius + otherRadius)) {
                return true;
            }
        }
    ...
    enableVRMode() {
        ...
        // Evento para estadísticas
        window.dispatchEvent(new CustomEvent('vr-mode-enabled'));
    }
    ...
    strafe(direction) {
        ...
        // Evento para estadísticas
        window.dispatchEvent(new CustomEvent('player-strafed', { 
            detail: { direction, newLane: this.currentLane }
        }));
    }
    ...
    _updateBoundingBox() {
        ...
        // Mantener bounding box para compatibilidad
        this.boundingBox.setFromObject(this.group, true);
        
        // Ajustar bounding box durante el roll
        if (this.state === Config.PLAYER_STATE.ROLLING) {
            this.boundingBox.min.y += 0.5;
        }
        
        this.boundingBox.expandByScalar(Config.COLLISION_SETTINGS.OBSTACLE_PADDING);
    }
    ...
    debugInfo() {
        return {
            ...
            vrMode: this.vrMode,
            hitSpheres: this.hitSpheres.length
        };
    }
}